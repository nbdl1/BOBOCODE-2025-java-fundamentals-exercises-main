...
public class HeterogeneousMaxHolder {
    ...
    private static Map<Class<?>, Object> heterogeneousMax = new HashMap<Class<?>, Object>();
    /**
    ...
    public static <T extends Comparable<? super T>> T put(Class<T> key, T value) {
        System.out.println("Current Max is "+heterogeneousMax.get(key));
        System.out.println("Provided for comparison "+value);
        if (heterogeneousMax.get(key)==null||!heterogeneousMax.containsKey(key)|| value.compareTo((T)heterogeneousMax.get(key)) > 0) {
            return(T) heterogeneousMax.put(key,value);
        }else{
            return value;
        }
    }
    ...
    public static <T> T put( Class<T> key,  T value,  Comparator<? super T> comparator) {
        T min = null;
        if(heterogeneousMax.get(key)!=null){
        min = comparator.compare(value,(T)heterogeneousMax.get(key)) > 0?(T)heterogeneousMax.get(key):value;
        }else {
            System.out.println("Provided value is "+value);
            heterogeneousMax.put(key,value);
            return null;}
        if (!heterogeneousMax.containsKey(key)|| comparator.compare(value,(T)heterogeneousMax.get(key)) > 0) {
            T returned = (T) heterogeneousMax.put(key,value);
    ...
    public static <T> T getMax(Class<T> key) {
        System.out.println("provided key is "+key);
        System.out.println("value is "+heterogeneousMax.get(key));
        return (T) heterogeneousMax.get(key);
    }

    public void clear(){
        heterogeneousMax.clear();
    }
    // todo: implement a method according to javadoc
}
