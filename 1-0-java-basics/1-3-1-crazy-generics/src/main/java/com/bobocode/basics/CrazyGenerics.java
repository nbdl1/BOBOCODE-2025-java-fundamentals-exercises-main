    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
           Iterator<? extends BaseEntity> iterator = entities.iterator();
            while(iterator.next().getUuid()!=null&&iterator.hasNext()){
                return false;
            }
            return true;
        ...
        public static <T extends BaseEntity>  boolean hasDuplicates(List<T> entities,T targetEntity) {
            return  entities.stream()
                    ...
                    .count()>1L;// todo: update method signature and implement it
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            final Optional<T>[] max = new Optional[]{Optional.of(elements.iterator().next())};
            elements.forEach(element -> {

                if(comparator.compare(element, max[0].get())>0){
                    max[0] =Optional.of(element);
                }
            });
            return max[0];
        }
        ...
        public static void swap(List<Object> elements, int i, int j) {
            ...
            //Maybe it's not that supoosed me to do))
